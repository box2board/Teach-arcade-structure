...
(function () {
  ...
  <style>
    ...
    .hamburger span {
      display:block; height:2px; margin:6px 0; background:#e5e7eb;
  ...
  function mountHeader() {
    ...
    // On mobile, tapping a dropdown-toggle should expand/collapse instead of navigating.
    const isMobile = () => window.matchMedia('(max-width: 880px)').matches;

    mount.querySelectorAll('.dropdown > a.dropdown-toggle').forEach(a => {
      a.addEventListener('click', (e) => {
        if (!isMobile()) return; // desktop: normal link + hover behavior
        e.preventDefault();
