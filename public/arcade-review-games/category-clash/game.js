(() => {
  ...
  function startGame(){
    ...
    state={soundOn:state?.soundOn!==false,mode:document.querySelector('input[name="playMode"]:checked')?.value||'classroom',scores:data.teams.map(()=>0),activeTeam:0,completed:{},current:null,powerWager:null,finalWagers:[],finalResults:[],individualCorrect:0,individualAnswered:0,individualMissed:[],individualAttempts:[],bestStreak:0,currentStreak:0,startedAt:Date.now()};
    renderBoard();show('boardView');sound('good');
  }
  function renderBoard(){
    $('boardTitle').textContent=data.title;$('scoreboard').innerHTML=(state.mode==='individual'?[data.teams[0]]:data.teams).map((t,i)=>`<button class="score-card ${state.mode==='classroom'&&i===state.activeTeam?'active':''}" style="--team:${t.color}" data-active-team="${i}" aria-label="Make ${escapeHtml(t.name)} active"><span>${escapeHtml(t.name)}</span><strong>${state.scores[i]}</strong></button>`).join('');
    $('turnAnnouncer').textContent=state.mode==='individual'?`${state.individualAnswered} answered · ${state.individualCorrect} correct · ${state.scores[0]} points`:`${data.teams[state.activeTeam].name} is the active team.`;
    ...
    $('gameBoard').innerHTML=html;const allDone=data.categories.every((c,ci)=>c.questions.every((_,qi)=>state.completed[`${ci}-${qi}`]));$('finalBtn').hidden=!(allDone&&data.finalEnabled);if(allDone&&state.mode==='individual')setTimeout(showWinner,250);else if(allDone&&!data.finalEnabled)setTimeout(showWinner,250);
  }
  function openQuestion(ci,qi){
    ...
    $('answeringTeam').innerHTML=(state.mode==='individual'?[data.teams[0]]:data.teams).map((t,i)=>`<option value="${i}" ${i===state.activeTeam?'selected':''}>${escapeHtml(t.name)}</option>`).join('');resetTimer();$('timerBox').hidden=!data.timerEnabled;show('questionView');$('questionText').focus();sound();
  }
  function revealAnswer(){ if(state.current&&state.powerWager!==null){$('answerArea').hidden=false;$('revealBtn').hidden=true;$('judgeControls').hidden=state.mode==='individual';$('individualControls').hidden=state.mode!=='individual';if(state.mode==='individual')renderIndividualControls();pauseTimer();sound();} }
  function judge(correct){ const team=+$('answeringTeam').value,q=data.categories[state.current.ci].questions[state.current.qi],value=q.power?state.powerWager:q.points;state.scores[team]+=correct?value:-value;if(state.mode==='individual'){state.individualAnswered++;state.currentStreak=correct?state.currentStreak+1:0;state.bestStreak=Math.max(state.bestStreak,state.currentStreak);if(correct)state.individualCorrect++;else state.individualMissed.push({ci:state.current.ci,qi:state.current.qi});state.individualAttempts.push({category:data.categories[state.current.ci].name,question:q.question,correct,points:q.points,answeredAt:new Date().toISOString()});}state.completed[`${state.current.ci}-${state.current.qi}`]=true;state.activeTeam=correct?team:(state.activeTeam+1)%data.teams.length;pauseTimer();sound(correct?'good':'bad');state.current=null;renderBoard();show('boardView'); }
  function cleanAnswer(v){return String(v||'').toLowerCase().replace(/[^a-z0-9 ]/g,'').replace(/^(what|who|where|when|why|how) is /,'').replace(/^(a|an|the) /,'').trim();}
  ...
  function submitIndividual(value){const q=data.categories[state.current.ci].questions[state.current.qi],accepted=(q.acceptedAnswers?.length?q.acceptedAnswers:[q.answer]).map(cleanAnswer),correct=accepted.includes(cleanAnswer(value));$('individualControls').innerHTML='<div class="feedback '+(correct?'correct-feedback':'incorrect-feedback')+'"><strong>'+(correct?'✓ Correct!':'✕ Not quite')+'</strong><p>'+escapeHtml(q.answer)+'</p><button id="continueIndividual" class="btn primary" type="button">Back to board →</button></div>';$('continueIndividual').onclick=()=>judge(correct);sound(correct?'good':'bad');}
  ...
  function wire(){
    ...
    $('gameBoard').onclick=e=>{const b=e.target.closest('[data-square]');if(b&&!b.disabled){const [ci,qi]=b.dataset.square.split(',').map(Number);openQuestion(ci,qi);}};$('scoreboard').onclick=e=>{const b=e.target.closest('[data-active-team]');if(b&&state.mode==='classroom'){state.activeTeam=+b.dataset.activeTeam;renderBoard();sound();}};
